...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
    ...
    /* renamed from: UP */
    public static final int f20UP = 1;
    int mActionState = 0;
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == 0) {
                ...
                if (ItemTouchHelper.this.mSelected == null) {
                    ...
                    if (animation != null) {
                        ItemTouchHelper.this.mInitialTouchX -= animation.f21mX;
                        ItemTouchHelper.this.mInitialTouchY -= animation.f22mY;
    ...
    public static abstract class Callback {
        ...
        private int getMaxDragScroll(RecyclerView recyclerView) {
            if (this.mCachedMaxScrollSpeed == -1) {
                this.mCachedMaxScrollSpeed = recyclerView.getResources().getDimensionPixelSize(C0249R.dimen.item_touch_helper_max_drag_scroll_per_frame);
            }
        ...
        public void onDraw(Canvas c, RecyclerView parent, ViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                ...
                anim.update();
                int count = c.save();
                onChildDraw(c, parent, anim.mViewHolder, anim.f21mX, anim.f22mY, anim.mActionState, false);
        ...
        public void onDrawOver(Canvas c, RecyclerView parent, ViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                RecoverAnimation anim = (RecoverAnimation) recoverAnimationList.get(i);
                int count = c.save();
                onChildDrawOver(c, parent, anim.mViewHolder, anim.f21mX, anim.f22mY, anim.mActionState, false);
                c.restoreToCount(count);
            }
            if (selected != null) {
                int count2 = c.save();
    ...
    private class RecoverAnimation implements AnimatorListenerCompat {
        ...
        public void update() {
            if (this.mStartDx == this.mTargetX) {
                this.f21mX = ViewCompat.getTranslationX(this.mViewHolder.itemView);
            } else {
                this.f21mX = this.mStartDx + (this.mFraction * (this.mTargetX - this.mStartDx));
            }
            if (this.mStartDy == this.mTargetY) {
                this.f22mY = ViewCompat.getTranslationY(this.mViewHolder.itemView);
            } else {
                this.f22mY = this.mStartDy + (this.mFraction * (this.mTargetY - this.mStartDy));
            }
        }
    ...
    public View findChildView(MotionEvent event) {
        ...
        for (int i = this.mRecoverAnimations.size() - 1; i >= 0; i--) {
            ...
            if (hitTest(view, x, y, anim.f21mX, anim.f22mY)) {
                return view;
            }
        }
